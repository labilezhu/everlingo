...
function ChannelsHome() {
  ...
  return (
    <div className="p-4 space-y-2">
      <h2 className="text-sm font-semibold text-foreground px-1">{t('channels')}</h2>
      <button
      ...
      >
        ...
        <span className="min-w-0">
          <span className="block text-sm font-medium text-foreground">{t('wechat_channel')}</span>
          <span className="block text-xs text-muted-foreground">{t('wechat_channel_desc')}</span>
        </span>
      </button>
    </div>
  );
    ...
    <div className="flex flex-col h-screen mx-auto max-w-md border-x border-border">
      <header className="flex items-center gap-2 px-3 py-2 md:px-4 md:py-3 border-b border-border bg-background shrink-0">
        ...
        <h1 className="text-lg font-semibold text-foreground">
          {inWechatAdmin ? t('wechat_channel') : t('workspace_console')}
        </h1>
      </header>
