...
4. 如果 `<version>` 是正式版本，即没有 rc 。 如果存在一个非空的 `docs/release-notes/v<version>/v<version>-rel-notes.zh-CN.md` 文件， 且不存在一个非空的 `docs/release-notes/v<version>/v<version>-rel-notes.md` 文件，那么把里面的中文内容，翻译成英语，写入 `docs/release-notes/v<version>/v<version>-rel-notes.md` 文件。

5. Check file `/home/labile/diy-log/home-lab/hp/everlingo/130-release.sh` exists. If not, the system will abort the release operation and tell the reason to the user. 
6. Ask for a confirm and run:
  ```bash
  EVERLINGO_VER=<version> mark-specific/local-deploy/130_deploy/130-release.sh
  ```

7. Find the <version> in the `/VERSION_HISTORY.yaml` file. And update state:
  ```yaml
  - version: <version>
  - state: released
  ```  

8. 在 `/VERSION_HISTORY.yaml` 最开头，插入下一版本号： 
   ```yaml
