...
while True:
    env = channel.recv_envelope()      # UserInputEnvelope | None
    if env is None: put(QuitEvent)
    else: put(UserMessage(envelope=env))
...
- `UserMessage(envelope)` — 来自 channel.recv_envelope() 的用户输入，携带结构化 `UserInputEnvelope`（详见 [envelope-spec.md](envelope-spec.md)）
