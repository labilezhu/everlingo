...
- **适用范围**：全部 14 个工具（`list_vaults`、`create_vault`、`session.configure`、9 个 fs 工具 `ls`/`read`/`write`/`append`/`grep`/`find`/`stat`/`mkdir`/`delete`/`tree`、`search`）的每次调用。
- **level**：`logging.DEBUG`。
- **logger**：`everlingo.mem.vault.mcp_server`（`logging.getLogger(__name__)`），日志经 root handler 写入 `$workspace/logs/everlingo.log`（见 [observability.md](/docs/impl-spec/observability.md)）。
