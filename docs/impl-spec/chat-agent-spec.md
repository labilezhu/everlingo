...
是**偏好而非命令**。即使 `task=translate`，用户仍可追问其他问题，LLM 应按消息实际内容识别意图。
...
你只读不写；需要写入时通过 `request_memory_extraction` 工具触发异步写入流程。
```
...
工具执行体仅累积 entries drafts（`MainAgent._pending_drafts`），不直接入队 —— 确保 LLM 多步工具循环中 `new_messages` 切片正确的唯一时机是 `invoke()` 末尾。`MainAgent.invoke()` 末尾用 `_pending_drafts` 构造完整的 `MemoryEntry`（补全系统字段/渲染对话文本）并入队 `MemoryWriterAgent`。见 [chat-agent-tools-spec.md](/docs/impl-spec/chat-agent-tools-spec.md) 的 `request_memory_extraction` 工具定义。
...
当用户表达"记住 / 记下 / 帮我记"某 `target_lang` 知识点（单词/短语/语法点/语用）时，Chat Agent **必须先在本轮回复中产出该知识点的实际内容**（释义/解释/用法/举例，按上文「查单词」「翻译」要求用 `dest_lang` 给出），**然后**再调用 `request_memory_extraction(entries=[...])` 并附"已提交笔记请求"提示。

**为什么不能只回"已提交笔记请求"**：

下游 Memory Writer Agent 的正文写作与 `conversation_context` 生成需要依赖 `new_messages` 中的 `AIMessage.content` 作为事实来源。如果 Chat Agent 只回"已提交笔记请求"而不产出释义，`new_messages` 中关于该知识点没有任何事实内容，Writer 无法生成有意义的笔记正文。纠正事项（用户写错被纠正）的场景天然满足事实来源（用户原句 + Agent 纠正都在本轮对话里），无需额外动作。
...
- **不经过** 异步写入流程（跳过 entries 构造，因为没有新知识点要写入）
...
与用户消息处理分开，不触发 typing hint，跳过 entries 构造（知识已被 Writer 写入）。
...
用户交互输入/输出文本的日志在 `Session` 层记录（`session.py`），前缀 `[ChatAgent]`，见 [session.md — 交互日志](/docs/impl-spec/session.md#交互日志)。
...
- Chat Agent 的 LLM 通过 `request_memory_extraction` 工具决定是否驱动记忆写入。
- 工具调用累积 entries drafts，`MainAgent.invoke()` 末尾补全系统字段后入队 `MemoryWriterAgent`。
