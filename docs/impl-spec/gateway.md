...
负责：
- 按启动模式（explicit flag 单 channel / 无参 config-driven 多 channel）创建相应的 `Session Acceptor` 。
...
# 启动 Wechat Session Acceptor（standalone，无 web）
gateway --channel_wechat

# 启动 Web Session Acceptor（+ idle WechatRuntime 供 console 控制）
gateway --channel_web

# 无参：config-driven 多 channel
# 读 everlingo.yaml.plugins.channels，凡节点存在且 enable!=false 的 channel 全部 in-process 启动
gateway
```
