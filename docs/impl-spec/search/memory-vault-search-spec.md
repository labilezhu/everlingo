...
src/everlingo/mem/vault/
  ...
  mcp_server/
    ...
    mcp_server.py       # MCP Streamable HTTP Server（内嵌 indexer 进程，子线程跑 uvicorn）；暴露 fs + search + session.configure 工具；URL 写 $workspace/indexer.mcp.url（见 valut-mcp-spec.md）
```
