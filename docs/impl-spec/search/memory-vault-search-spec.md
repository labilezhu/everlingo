...
- 对 `$workspace/memory/languages/$lang/vault/` 下的 markdown vault 提供全文检索。每个目标学习语言独立一个 vault 与一个独立的 SQLite DB（见 [workspace.md](/docs/impl-spec/worksplace/workspace.md)「Workspace 目录结构」）。
- 索引对象分两类：
  - **kb items**（`$lang_vault/items/{type}/{slug}--{ulid}.md`）：字段化最强，主要检索对象。
  - **events**（`$lang_vault/events/YYYY/MM/YYYY-MM-DD.md`）：按天聚合的多事件文件，次要检索对象。
- 全文检索（FTS）≠ 语义检索。本文仅覆盖 FTS；语义检索见上。
- **语言隔离**：每个 lang 一个独立 DB，FTS BM25 词项统计与排名不跨语言互相影响。`lang` 列在 schema 中删除（已隐含于 DB），`SearchHit.lang` 字段由 indexer 按 DB 语言回填，protocol 对外不变。
...
采用**独立 indexer 进程**方案（方案 A）。一个 workspace 一个 indexer 进程，持有 N 个 lang SQLite 连接（每 lang 一个 RW 连接）；gateway 与 CLI 均为 indexer 的 HTTP 客户端，不直接触碰 SQLite。
...
│  Gateway 进程（可多个）  │  ─── $workspace/indexer.sock  │  Indexer 进程（单例前台，一个 workspace 一个） │
│  - Chat Agent           │                                │  - N 个 SQLite RW (WAL)，每 lang 一个        │
│  - Memory Writer Agent  │                                │    $workspace/memory/languages/$lang/index/  │
│  - SearchClient (thin)  │  ◄── SearchHit JSON ─────      │      memory.sqlite                            │
│  不碰 SQLite            │                                │  - N 个 watchdog watcher，每 lang vault 一个  │
│  不加载 mecab/jieba     │                                │  - 1 个 lang 发现 watcher（监听 languages/）   │
│  不加载 embedder        │                                │  - 启动对账 + 运行时新 lang 自动开 DB         │
└─────────────────────────┘                                │  - tokenizer(jieba+mecab)                     │
                                                            │  - FastAPI server                             │
                                                            │  - embedding worker (单 worker 轮询 N 个 DB)  │
                                                            │    (见 embedding-spec)                        │
                                                            │  - MCP Streamable HTTP Server (内嵌)          │
                                                            │    暴露 fs + search + session.configure 工具  │
                                                            │    URL 写 $workspace/indexer.mcp.url          │
                                                            │    (见 vault-mcp-spec.md)                      │
                                                            └──────────────────────────────────────────────┘
                                                                          │
                                                                          ▼
                                                              $workspace/memory/languages/$lang/index/memory.sqlite
                                                              （每语言一个 DB 文件）
```
...
- **单进程持多 DB**：所有 lang 共享一个 indexer 进程、一个 socket、一份 tokenizer/embedder 加载。相比「每 lang 一个 indexer 进程」节省进程开销与常驻内存，且 gateway 侧只需连一个 socket。
...
src/everlingo/mem/vault/
  ...
  mcp_server/
    __init__.py         # 导出 create_mcp_app / run_mcp_server / pick_free_port / SessionState / SessionRegistry
    mcp_server.py       # MCP Streamable HTTP Server（内嵌 indexer 进程，子线程跑 uvicorn）；暴露 fs + search + session.configure 工具；URL 写 $workspace/indexer.mcp.url（见 vault-mcp-spec.md）
```
...
每个目标学习语言独立一份 DB 与 vault，路径基于当前 workspace 解析（见 [workspace.md](/docs/impl-spec/worksplace/workspace.md)「Workspace 选择机制」）。`$lang` 为目标学习语言编码（en/ja/...）：

| 用途 | 路径 |
|---|---|
| DB | `$workspace/memory/languages/$lang/index/memory.sqlite` |
| 监听根 | `$workspace/memory/languages/$lang/vault/`（仅 `.md`） |
| IPC socket | `$workspace/indexer.sock`（workspace 级共享，不随 lang 拆分） |

- DB 与 vault 同 workspace 根、与用户内容分离，便于备份
- 只有 indexer 进程以读写方式打开 SQLite；gateway 与 CLI 永不直接打开
- 每个 workspace 一个 indexer 进程实例（一个共享 socket + N 个 lang DB），不可跨 workspace。同一台机跑多 workspace 时各起一个 `everlingo mem indexer start` 进程
- indexer 启动时枚举 `$workspace/memory/languages/*/` 确定语言集合，对每个存在的 lang 目录打开对应 DB；无 lang 目录时为空 workspace，indexer 仍可启动
- **运行时新 lang 发现**：indexer 启动后还会在 `$workspace/memory/languages/` 上跑一个 lang 发现 watcher（recursive）—— 一旦检测到新 `*/vault/` 目录出现，立即触发与启动时相同的 `open_lang` 流程（开 DB + reconcile + 启 per-lang watcher + embedder/worker）。同时 `POST /{lang}/index` 等端点在 miss 时会懒加载同一 `open_lang` 路径（vault 目录存在为前提），覆盖 Writer 写第一个 kb item 时顺带创建 lang vault 的场景。两条路径共享同一加锁 open 入口，同一 lang 不会重复打开。vault 目录从未创建过则端点返回 404 `lang not found`。
...
- **FTS 层**：整文件一行入库，`kind='event'`，`ulid` 用合成键 `event:{lang}:{date}`（如 `event:ja:2026-06-26`，`{lang}` 由 indexer 按 DB 语言注入，不来自文件路径）。
...
HTTP/1.1 over unix domain socket，REST + JSON。uvicorn 绑定 `$workspace/indexer.sock`（workspace 级共享），FastAPI 提供路由与 pydantic 模型校验，自带 `/docs` 交互文档便于浏览器调试。所有端点通过 path segment `/{lang}/...` 路由到对应语言的 DB（详见 [search-api-spec.md](/docs/impl-spec/search/search-api-spec.md)）。

每个目标学习语言独立一个 vault 与一个 SQLite DB（见 [workspace.md](/docs/impl-spec/worksplace/workspace.md)「Workspace 目录结构」、[memory-vault-search-spec.md](/docs/impl-spec/search/memory-vault-search-spec.md)）。HTTP 端点通过 path segment `/{lang}/...` 路由到对应语言的 DB。`lang` 为必填，不支持跨语言检索（不传 `lang` 返回错误）。
...
- indexer 进程：FastAPI app + uvicorn `--uds $workspace/indexer.sock`。SearchHit/ChunkRef 用 pydantic 模型，序列化自动完成。`SearchHit.lang` 由 indexer 按路由 lang 回填（不来自 documents 列）。
...
indexer 进程对每个存在的 lang vault 各起一个 watchdog watcher，监听 `$workspace/memory/languages/$lang/vault/` 下的 `.md` 增删改，事件路由到 indexer（携带 lang 上下文，写入对应 lang DB）。watcher **排除** `$lang/vault/tmp/` 子目录（程序内部临时文件，无用户数据价值，不应索引；见 [vault_spec.md](/src/everlingo/mem/vault/vault_spec.md)）：
...
`Memory Writer Agent` 写完 `.md` 后，经 `SearchClient` 向 indexer 发 `POST /{lang}/index` 请求，**fire-and-forget**（失败靠 watcher + indexer 重启后启动对账兜底）。gateway 进程不直接调 indexer.py，不打开 SQLite。

### 启动时全量对账（indexer 进程内）

indexer 进程启动先枚举 `$workspace/memory/languages/*/`，对每个存在的 lang vault 扫一遍（排除 `tmp/` 子目录），用 `file_mtime` 与 `content_hash` 对账（补漏 + 清孤儿行），再启动该 lang 的 watcher。覆盖 watcher 漏掉的事件（如 indexer 未运行期间的外部编辑）。同时比对各 lang DB 的 `meta.tokenizer_version`，版本变化则全量重建该 lang 的 FTS。

### 运行时新 lang 发现（lang 发现 watcher + 端点懒加载）

启动后可能新增 lang 目录（典型场景：用户首次学某语言，Writer 写第一个 kb item 时顺带创建 `$workspace/memory/languages/$lang/vault/`）。indexer 用两条路径覆盖：

1. **lang 发现 watcher**（indexer 启动后常驻）：`Observer` 监听 `$workspace/memory/languages/`（recursive），仅响应 `*/vault/` 目录的 `on_created` 事件。检测到后调 `AppState._open_lang(lang)` 走与启动时一致的 `open()` 流程（开 DB + reconcile + per-lang watcher + embedder/worker）。
2. **端点懒加载**：`POST /{lang}/index` 等路由 miss 时调 `AppState._open_lang(lang)`。同 `workspace.lang_dirs()` 一致，vault 目录必须存在才开；不存在返回 404 `lang not found`。这条路径覆盖 Writer 写文件 → 立刻 `POST /{lang}/index` 的主流程，watcher 来不及就先补上。

两条路径共享 `AppState._open_lang()` 同一加锁入口；同一 lang 重复触发仅开一次。

### 数据流

```
Writer 写 .md ─► SearchClient.index_file(lang, path) ─HTTP /{lang}/index─► indexer
                                                               │
                                                   indexer 进程 ├─► documents + documents_fts (文件级，写入 $lang DB)
                                                               │     └─ 切 section ──► chunks (段级, 无 embedding)
                                                               │
                                                   N 个 watchdog watcher (每 lang vault 一个, mtime 去抖, ulid 幂等 upsert)
                                                               │
                          Lang 发现 watcher (监听 memory/languages/, recursive; 检测新 lang/vault/ → 调 _open_lang → 开 DB + reconcile + per-lang watcher)
                                                               │
indexer 启动 ─► 枚举 languages/*/ → 对每个 lang 全量对账 (scan + content_hash diff + tokenizer 版本比对) ─► 启动该 lang 的 watcher ─► 启 lang 发现 watcher
...
class SearchClient:
    ...
    def status(self, lang: str | None = None) -> dict: ...    # lang=None 聚合所有语言 DB 状态
```

- `lang` 为必填（不再支持 `lang=None` 跨语言检索；传 `None` 由 API 返回错误）。gateway 调用方按当前会话的目标学习语言传入。
- indexer 不可达时 `search()` 返回 `[]` + log warning；`index_file()` 返回 `False` + log warning。

### indexer 侧接口（search.py，进程内直接查对应 lang 的 SQLite）
...
everlingo mem reindex [LANG] [PATH]      # LANG 指定语言（必填）；PATH 为该 lang vault 下的文件或目录；省略 PATH 则全该 lang vault

# 完全删除指定 lang 的 index 数据，从零重建
everlingo mem reindex LANG --rebuild
...
| 命令 | 行为 |
|---|---|
| `indexer start` | 前台启动 indexer 进程（FastAPI + uvicorn `--uds $workspace/indexer.sock`），独占 N 个 lang SQLite RW，跑 N 个 watcher + 启动对账；阻塞当前终端，日志写 `$workspace/logs/indexer.log`，Ctrl-C 退出。 |
| `indexer status` | 经 `GET /status` 查询运行状态、各 lang DB 的文档数/chunk 数/tokenizer 版本。 |
| `reindex [LANG] [PATH]` | 经 `POST /{lang}/index` 批量投递 PATH 下 `.md`，由 indexer 比对 `file_mtime`+`content_hash`：未变 skip，已变 upsert，清孤儿。PATH 省略 = 全该 lang vault。 |
| `reindex LANG --rebuild` | 经 `POST /{lang}/rebuild`，indexer 删除该 lang DB 文件 → 重新 init schema → 全量扫描重建。 |
