...
│  Gateway 进程（可多个）  │  ─── $workspace/memory/vault_index/indexer.sock     │  Indexer 进程（单例前台） │
│  - Chat Agent           │                            │  - SQLite RW (WAL)        │
...
└─────────────────────────┘                            │    (见 embedding-spec)   │
                                                       └──────────────────────────┘
                                                                 │
                                                                 ▼
                                                         $workspace/memory/vault_index/memory.sqlite
```
...
| 用途 | 路径 |
|---|---|
| DB | `$workspace/memory/vault_index/memory.sqlite` |
| 监听根 | `$workspace/memory/vault/`（仅 `.md`） |
| IPC socket | `$workspace/memory/vault_index/indexer.sock` |
...
HTTP/1.1 over unix domain socket，REST + JSON。uvicorn 绑定 `$workspace/memory/vault_index/indexer.sock`，FastAPI 提供路由与 pydantic 模型校验，自带 `/docs` 交互文档便于浏览器调试。
...
- indexer 进程：FastAPI app + uvicorn `--uds $workspace/memory/vault_index/indexer.sock`。SearchHit/ChunkRef 用 pydantic 模型，序列化自动完成。
