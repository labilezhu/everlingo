...
- **过滤策略**：vec0 不支持复杂 WHERE。先取 `k * 3` 候选，再 join `chunks`+`documents` 按 item_type/kind/tags post-filter，取 top-k。无过滤时直接 top-k。lang 过滤已隐含于 DB，不再需要。
   - **vec0 KNN 语法**：`WHERE embedding MATCH ? AND k = ? ORDER BY distance`（使用 `k = ?` 而非 `LIMIT ?`，因为 `LIMIT ?` 绑定参数仅 SQLite >=3.43 起传给 virtual table xBestIndex；容器基镜像 `python:3.12-trixie` 链接系统 SQLite 3.46.1 已支持该行为，`k = ?` 仍为 sqlite-vec 官方推荐写法，两端兼容）。
