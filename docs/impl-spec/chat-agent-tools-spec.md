...
function description: 请求将本轮对话中的知识点异步写入记忆库。调用后立即返回，异步执行不阻塞回复。
parameters:
    entries: list of objects 。每个对象含 item_type / why_want_to_save_memory / title。
    entries[].item_type: string（枚举）。vocab / phrase / grammar / pragmatics / others
    entries[].why_want_to_save_memory: string（枚举）。用户明确要求记住知识点 / 纠正事项 / Chat Agent 判定
    entries[].title: string 。使用界面语言，限一句话，描述本知识点。用于语义搜索与全文搜索。
...
**实现机制**：
- 工具执行体仅把 entries drafts 累积到 `MainAgent._pending_drafts`，不在工具调用循环内直接入队。
- 实际构造 `MemoryEntry`（补全系统字段/渲染对话文本）并入队 `MemoryWriterAgent` 由 `MainAgent.invoke()` 末尾统一处理，确保 `new_messages` 切片正确。
...
- `request_memory_extraction` 触发**异步** Chat Agent → Memory Writer 创建/合并流程，工具立即返回。
- `memory_writer_action` 触发**同步** delete/edit 流程，**不经过** 异步写入流程，工具阻塞等待结果。
