...
> 2026-07 设计变更：原计划让 wechat gateway 跑在独立子进程、web 经 UDS admin socket 管理。经评估，子进程模型维护成本超过收益，改为 **web 进程内 in-process 托管**（`WechatRuntime` 实现 `SessionAcceptor`）。配合 `everlingo.yaml` 的 `channel_wechat.enable` 配置实现重启后自动恢复。详见 [architecture.md §1.1](./architecture.md)。
