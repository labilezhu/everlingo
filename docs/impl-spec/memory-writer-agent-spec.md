...
System prompt 还需要包括 src/everlingo/mem/vault/vault_specs/default/mem_entry_spec.md ，用于告知 Agent 其输入 entry 的完整字段结构与字段含义（字段补充说明）。同样通过 `PackageSource` + `compile_prompt` 加载。
