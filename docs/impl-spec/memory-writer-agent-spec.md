...
3. `## memory vault 结构规范` — vaule_spec.md 编译展开后的 vault 目录结构与文件格式规范
4. `# memory vault 注意事项` 及其子节 — vault 操作规范

注入 `vault_spec.md` 后，system prompt 中不再包含「调用 `read(spec/vault_spec.md)`」的指令；
LLM 在处理某类知识点时可调用 `read(path="spec/kb_items_spec_<type>.md")` 加载对应子规范。
