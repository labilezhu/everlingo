...
- `tools` 为 langchain `BaseTool` 列表（来自 `load_mcp_tools(session)` 过滤后的子集 + 客户端 `mem_gen_id`）。
...
- `mem_gen_id()` 返回类似 01JZABD123 格式的 随机 id（26 字符 ULID）。客户端纯计算工具，不走 MCP server。可用于 markdown 文件名部分。
