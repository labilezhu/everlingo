...
### Agent tools
所有文件和目录操作，都只能使用相对 path。假设当前目录位于 $workspace/memory/vault/ 目录。

工具沙箱：
mem_* 工具"只能用相对 path，假设当前目录位于 $workspace/memory/vault/"。必须在工具层强制校验：解析后路径不能逃出 vault_dir()（防 ../）。否则 LLM 一次幻觉就写到 workspace 外。
