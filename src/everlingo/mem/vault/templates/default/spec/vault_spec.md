...
- `触发知识点用户消息envelope`: `触发知识点用户消息` 所包含的消息和消息发出时的用户操作场景([Envelope 结构化用户输入格式](envelope_spec.md))。指 `触发知识点用户消息` 中的 `<envelope></envelope>` 包含的 json 内容。其中很多信息可以作为知识点的来源。
...
当当前 `知识点` 的 `触发知识点用户消息envelope` 中以下字段有值:
 - `source.url`
 - `source.title`
相应的 `遇到记录` 需要有来源的链接。示例见上面示例中 `遇到记录` 中的 `2026-06-21 12:01:02` 记录。
