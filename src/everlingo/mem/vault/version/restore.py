...
def restore_vault(
    ...
    *,
    ...
    if not git.git_available() or not git.is_repo(root):
        return RestoreResult(ok=False, message="memory repo 未初始化，无法恢复")

    # 1. 捕获本地改动
    try:
        git.add_and_commit(root, "chore(vault): pre-restore snapshot")
    except git.GitError as e:
        return RestoreResult(ok=False, message=f"pre-restore commit 失败: {e}")

    # 2. fetch
    try:
        git.ensure_remote(root, remote_url)
        git.fetch(root, env=env, config=config)
    except git.GitError as e:
        return RestoreResult(ok=False, message=f"fetch 失败: {e}")

    # 3. rebase 到远端
    ok, conflicts = git.rebase(root, f"origin/{branch}")
    if ok:
        return RestoreResult(ok=True, message="已 rebase 到远端")

    # 4. 冲突 → 备份分支（不覆盖工作区），交由用户决定 hard reset
    try:
        backup = git.checkout_to_backup_branch(root, "HEAD")
    except git.GitError:
        backup = None
    return RestoreResult(
        ...
        conflicts=conflicts,
        message="远端与本地冲突，已保存到 backup 分支，请确认后 hard reset",
    )
...
def hard_reset_to_remote(
    ...
    *,
    ...
    try:
        ...
        git.ensure_remote(root, remote_url)
        git.fetch(root, env=env, config=config)
        git.hard_reset(root, "origin", branch)
        return True
    except git.GitError as e:
