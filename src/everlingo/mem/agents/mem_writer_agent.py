...
def _build_writer_system_prompt(mem_entry_spec_content: str, envelope_spec_content: str, vault_spec_content: str) -> str:
    ...
    vault_spec_doc = shift_headings(
        vault_spec_content,
        offset=2,
    ).strip()

    prompt = f"""你是 EverLingo 的 Memory Writer Agent。Chat Agent 会把构造好的
conversation memory entries 异步转交给你，由你把每个 entry 合并或写入 memory vault。
你**不**与用户对话。你**不**接受外部输入（除 Chat Agent 转交的 entry 外）。
...
本节是 vault 目录结构规范和各类文件格式说明，原文件位于 /spec/vault_spec.md 。其中如果有你需要进一步了解的内容，并且是以 markdown 链接提供的，务必调用 read(path=$link_path) 按需加载。 由于原文件位于 /spec/vault_spec.md ，调用 read 时， 注意链接的相对路径，如 链接是`[vocab](kb_items_spec_vocab.md)`， 调用 read 时，应该是 `read("/spec/kb_items_spec_vocab.md")` 而不是 `read("kb_items_spec_vocab.md")`
...
典型用法： `search(q="...", mode="hybrid")` → `read(path=<hit.file_path>)` → `write(path=..., content=...)`。
...
2. **如果已有该条目文件**：
    ...
    2. 按 `vault_spec.md` 要求，在内存中合并 entry 的内容到条目文件。
    3. `write(file_path, new_content)` 写入。
 3. **如果未命中**（新建条目）：
     1. `vault_mcp_gen_id()` 取一个 ULID（26 字符）作为 frontmatter `ulid`（文件名不含 ulid）。
     2. 按 `vault_spec.md` 要求，构造 slug；写入前先用 `ls` / `search` 确认无同名冲突。
     3. 文件的命名和保存的目录位置按 `vault_spec.md` 要求。
    4. 按 `vault_spec.md` 中对应 `知识类型` 的模板构造 frontmatter + 正文。
...
class MemoryWriterAgent:
    ...
    - `enqueue(entries)` 仅入队、不阻塞，由 Chat Agent 调用。
    ...
    def enqueue(self, entries: list[MemoryEntry]) -> None:
        ...
        ref: Chat Agent ainvoke 末尾构造 MemoryEntry 后入队
        """
    ...
    def _run_loop(self) -> None:
        ...
        队列中可能包含两种 item：
        - list[MemoryEntry]：由 Chat Agent enqueue（create 流程）
