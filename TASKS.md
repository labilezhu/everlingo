...
- 2026-07-29 | 修复 `everlingo ws_master --config <path> user add` 误启动 daemon 而非 CLI：`main.py:245` 分派顺序中 `args.config` 优先于 `args.ws_master_cmd`，导致带 `--config` 执行子命令时走 daemon 分支（FastAPI 绑定 8101），报端口占用。改为 `args.ws_master_cmd` 优先——有子命令时走 `cli.dispatch`（`--config` 仅用于定位 sqlite），无子命令才走 daemon 模式。
